...
public class Carousel extends CarouselSpinner implements GestureDetector.OnGestureListener {
	...
	public void abc (Context context, AttributeSet attrs, int defStyle) {
	...
	//	TypedArray arr = getContext().obtainStyledAttributes(attrs, R.styleable.Carousel);
		...
		mUseReflection = true;//arr.getBoolean(R.styleable.Carousel_UseReflection, false);	
		int selectedItem = 0;//arr.getInteger(R.styleable.Carousel_SelectedItem, 0);
		...
		adapter.SetImages(mUseReflection);
...
/*		    String ExternalStorageDirectoryPath = Environment
			...
			Toast.makeText(mContext, "Number of images = " + files.length, Toast.LENGTH_SHORT).show();*/
			
			int imagesFound = MilestonesDbAdapter.glstMilestoneInformation.size();
			Toast.makeText(mContext, "Actual images found = " + MilestonesDbAdapter.glstMilestoneInformation.size(), Toast.LENGTH_SHORT).show();
			
			//mImages = new CarouselItem[files.length];
			mImages = new CarouselItem[imagesFound];
			
			//for (File file : files){
			for (int index=0; index < imagesFound; index++){
					
				//String path = file.getAbsolutePath();
				//Uri uri = Uri.parse("android.resource://com.example.uibasics/drawable/post_it_note.jpg");
				...
				Bitmap originalImage = BitmapFactory.decodeResource(mContext.getResources(),
		                    R.drawable.rabbit, options);
				//Bitmap originalImage = decodeSampledBitmapFromUri(path1, 220, 220);
				
				//	Modify this bitmap and use the new one 
				...
				canvas.drawText(information.getCaption(), originalImage.getWidth()/2-20, originalImage.getHeight()/2-40, mTextPaint);
				//Toast.makeText(mContext, "Shilpa " + i, Toast.LENGTH_LONG).show();
				...
				/*if(names != null)
				{
					String what = names.getString(i);
					Toast.makeText(mContext, "What is this ->" + what, Toast.LENGTH_LONG).show();
    ...
    private void Calculate3DPosition(CarouselItem child, int diameter, float angleOffset){
    	...
    	float x = - (float)(diameter/2  * java.lang.Math.sin(angleOffset)) + diameter/2 - child.getWidth()/2;
    	float z = diameter/2 * (1.0f - (float)java.lang.Math.cos(angleOffset));
    	float y = - getHeight()/2 + (float) (z * java.lang.Math.sin(mTheta));
    	//float mVerticalStretchFactor = 140f;
    	//float y = - getHeight()/2 + (float) (diameter/mVerticalStretchFactor * z * java.lang.Math.sin(mTheta));
